...
<!-- 왼쪽메뉴 선택된 부분 활성화 시키는 J쿼리 명령어 추가(아래)-->
<script>
$(document).ready(function(){
	//현재 선택된 URL값을 가져오는 명령(아래)
	var current = location.pathname;//current변수저장소에서 board,member 클릭한 내용확인
	var current_2 = current.split("/")[2];//스플릿분리함수로 current안에 있는 문자를 분리한 배열값을 반환.
	//alert(current_2);//디버그용
	//alert(current);//admin/(대시보드-undefined),admin/member_list(관리자관리)
	//admin/board_list(게시물관리)
	//active클래서명을 동적으로 추가할 영역은 nav-item안쪽의 a태그의 클래스명을 추가하면 됨
